...
def forecast_future(data, model, time_step=3, n_future=3):
    ...
    for _ in range(n_future):
        next_pred = model_predict(model, current_input)[0].numpy()
...
def calculate_mape(actual, forecast, epsilon=1e-10):
    ...
    # Sıfır olmayan değerler üzerinde işlem yap
    mask = actual != 0
    actual = np.where(mask, actual, epsilon)
    forecast = np.where(mask, forecast, epsilon)
    return np.mean(np.abs((actual - forecast) / actual)) * 100

# Function to prepare the data for LSTM for the first 50 rows of the dataset
def prepare_lstm_data(df, time_step=3, rows=50):
    lstm_predictions = []
    mape_scores = []
    ...
    for i in range(rows):
        ...
        # Train the model
        model.fit(X_train, Y_train, epochs=25, batch_size=32, verbose=0, callbacks=[early_stopping])
        ...
        lstm_predictions.append(np.round([future_predictions[0][0], future_predictions[1][0], future_predictions[2][0]], 2))
        mape_scores.append(np.round([mape_2020, mape_2021, mape_2022], 2))
    
    return lstm_predictions, mape_scores

# Function to forecast using ARIMA model
def arima_forecasting(data, start_year_column, rows=50):
    ...
    for index, row in data.iloc[:rows].iterrows():
        ...
        pred_2020.append(np.round(forecast[0], 2))
        pred_2021.append(np.round(forecast[1], 2))
        pred_2022.append(np.round(forecast[2], 2))
        
        mape_2020.append(np.round(mean_absolute_percentage_error([row['2020']], [forecast[0]]), 2))
        mape_2021.append(np.round(mean_absolute_percentage_error([row['2021']], [forecast[1]]), 2))
        mape_2022.append(np.round(mean_absolute_percentage_error([row['2022']], [forecast[2]]), 2))

    return pred_2020, pred_2021, pred_2022, mape_2020, mape_2021, mape_2022

# Function to forecast using Prophet model
def prophet_forecasting(data, start_year_column, rows=50):
    pred_2020, pred_2021, pred_2022 = [], [], []
    mape_2020, mape_2021, mape_2022 = [], [], []
    ...
    for index, row in data.iloc[:rows].iterrows():
        ...
        pred_2020.append(np.round(forecast_2020[0], 2) if len(forecast_2020) > 0 else None)
        pred_2021.append(np.round(forecast_2021[0], 2) if len(forecast_2021) > 0 else None)
        pred_2022.append(np.round(forecast_2022[0], 2) if len(forecast_2022) > 0 else None)
        
        if len(forecast_2020) > 0:
            mape_2020.append(np.round(mean_absolute_percentage_error([row['2020']], [forecast_2020[0]]), 2))
        else:
            mape_2020.append(None)
        
        if len(forecast_2021) > 0:
            mape_2021.append(np.round(mean_absolute_percentage_error([row['2021']], [forecast_2021[0]]), 2))
        else:
            mape_2021.append(None)
        
        if len(forecast_2022) > 0:
            mape_2022.append(np.round(mean_absolute_percentage_error([row['2022']], [forecast_2022[0]]), 2))
        else:
...
# Apply ARIMA model
arima_pred_2020, arima_pred_2021, arima_pred_2022, arima_mape_2020, arima_mape_2021, arima_mape_2022 = arima_forecasting(df, start_year_column, rows=50)

# Apply Prophet model
prophet_pred_2020, prophet_pred_2021, prophet_pred_2022, prophet_mape_2020, prophet_mape_2021, prophet_mape_2022 = prophet_forecasting(df, start_year_column, rows=50)

# Prepare LSTM data
lstm_predictions, mape_scores = prepare_lstm_data(df, rows=50)

# Zaman sayacını durdur ve geçen süreyi hesapla
end_time = time.time()
