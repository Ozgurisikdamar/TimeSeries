...
def process_time_series(file_path, start_year_column, rows=None):
    # Load the data
    ...
    # Function to prepare the data for LSTM for the specified number of rows or the entire dataset
    def prepare_lstm_data(df, time_step=3, rows=None):
        lstm_predictions = []
        mape_scores = []
    ...
    def arima_forecasting(data, start_year_column, rows=None):
    ...
    def prophet_forecasting(data, start_year_column, rows=None):
        pred_2020, pred_2021, pred_2022 = [], [], []
        mape_2020, mape_2021, mape_2022 = [], [], []
        ...
        for index, row in data.iloc[:rows].iterrows():
            ...
            # Ensure future DataFrame includes dates up to 2022
            ...
            # Ensure predictions are non-negative
            forecast_2020 = np.maximum(forecast_2020, 0)
            forecast_2021 = np.maximum(forecast_2021, 0)
            forecast_2022 = np.maximum(forecast_2022, 0)
    ...
    # Apply ARIMA model
    arima_pred_2020, arima_pred_2021, arima_pred_2022, arima_mape_2020, arima_mape_2021, arima_mape_2022 = arima_forecasting(df, start_year_column, rows=rows)

    # Apply Prophet model
    prophet_pred_2020, prophet_pred_2021, prophet_pred_2022, prophet_mape_2020, prophet_mape_2021, prophet_mape_2022 = prophet_forecasting(df, start_year_column, rows=rows)

    # Prepare LSTM data
    lstm_predictions, mape_scores = prepare_lstm_data(df, rows=rows)
...
rows = 100  # Example number of rows to process, set to None to process all rows

process_time_series(file_path, start_year_column, rows)
